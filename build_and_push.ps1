...
# Header
Write-Host "Docker LightRAG WatsonX - Optimized Build and Push Script" -ForegroundColor Magenta
Write-Host ("=" * 60)
...
if (-not $NoBuild) {
    ...
    $ImageInfo = docker images $FullImage --format "table {{.Repository}}:{{.Tag}}`t{{.Size}}`t{{.CreatedAt}}"
...
# Final summary
$BuildEnd = Get-Date
$TotalTime = [math]::Round(($BuildEnd - $BuildStart).TotalSeconds)
Write-Host ""
