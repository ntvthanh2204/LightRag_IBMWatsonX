...
class TokenTracker:
    """Track token usage and timing for LLM calls with detailed per-call logging."""

    def __init__(self, log_each_call=True):
        self.log_each_call = log_each_call
        self.reset()
    ...
    def reset(self):
        ...
        self.call_count = 0
        self.total_time = 0.0
        self.call_history = []  # Detailed history of each call
        self.min_time = float('inf')
        self.max_time = 0.0

    def add_usage(self, token_counts, call_time=None, model_name=None, operation_type=None):
        """Add token usage from one LLM call with detailed timing.

        Args:
            token_counts: A dictionary containing prompt_tokens, completion_tokens, total_tokens
            call_time: Time taken for this call in seconds
            model_name: Name of the model used
            operation_type: Type of operation (e.g., 'entity_extraction', 'query', 'relationship')
        """
        from datetime import datetime
        ...
        if call_time is not None:
            ...
            self.min_time = min(self.min_time, call_time)
    ...
    def get_usage(self):
        """Get current usage statistics with timing details."""
        avg_time = self.total_time / max(self.call_count, 1) if self.call_count > 0 else 0
        avg_tokens_per_call = self.total_tokens / max(self.call_count, 1) if self.call_count > 0 else 0
        overall_tokens_per_second = self.total_tokens / max(self.total_time, 0.001) if self.total_time > 0 else 0
    ...
    def get_call_history(self):
        """Get detailed history of all calls."""
    ...
    def get_timing_stats(self):
        ...
        times = [call["call_time"] for call in self.call_history if call["call_time"] is not None]
        if not times:
            return {}
    ...
    def export_call_history(self, filename=None):
        """Export detailed call history to JSON file."""
        import json
        from datetime import datetime
        
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"llm_call_history_{timestamp}.json"
        ...
        logger.info(f"📋 LLM call history exported to: {filename}")
        return filename

    def __str__(self):
        usage = self.get_usage()
        if usage['call_count'] == 0:
            return "LLM Tracker: No calls recorded"
            
        return (
            f"LLM Calls: {usage['call_count']} | "
            f"Total Time: {usage['total_time']:.2f}s | "
            f"Avg Time: {usage['avg_time_per_call']:.2f}s | "
            f"Total Tokens: {usage['total_tokens']:,} | "
            f"Speed: {usage['overall_tokens_per_second']:.1f} tokens/s | "
            f"Range: {usage['min_time']:.2f}s - {usage['max_time']:.2f}s"
        )


class EmbeddingTracker:
    """Track embedding API usage including texts processed, timing, and dimensions with detailed per-call logging."""
    ...
    def add_usage(self, texts_count: int, total_chars: int = 0, tokens_used: int = 0, 
                  embedding_dim: int = 0, call_time: float = None, model_name: str = None,
                  operation_type: str = None, batch_size: int = None):
        """Add embedding usage from one API call with detailed timing.

        Args:
            texts_count: Number of texts processed
            total_chars: Total characters in all texts
            tokens_used: Total tokens used for embedding
            embedding_dim: Dimension of embedding vectors
            call_time: Time taken for this call in seconds
            model_name: Name of the embedding model used
            operation_type: Type of operation (e.g., 'entity_embedding', 'chunk_embedding')
        ...
        chars_per_second = total_chars / call_time if call_time and call_time > 0 else None
        texts_per_second = texts_count / call_time if call_time and call_time > 0 else None
        tokens_per_second = tokens_used / call_time if call_time and call_time > 0 else None

        # Record detailed call history
        call_record = {
            "timestamp": datetime.now().isoformat(),
            "call_number": self.embedding_calls,
            ...
            "avg_chars_per_text": total_chars / max(texts_count, 1),
            "avg_tokens_per_text": tokens_used / max(texts_count, 1),
    ...
    def get_usage(self):
        """Get current embedding usage statistics with timing details."""
        avg_time = self.total_time / max(self.embedding_calls, 1) if self.embedding_calls > 0 else 0
        avg_texts_per_call = self.total_texts / max(self.embedding_calls, 1) if self.embedding_calls > 0 else 0
        overall_texts_per_second = self.total_texts / max(self.total_time, 0.001) if self.total_time > 0 else 0
        overall_tokens_per_second = self.total_tokens / max(self.total_time, 0.001) if self.total_time > 0 else 0
        ...
        return {
            ...
            "min_time": self.min_time if self.min_time != float('inf') else 0,
            "max_time": self.max_time,
            "avg_chars_per_text": self.total_characters / max(self.total_texts, 1),
            "avg_tokens_per_text": self.total_tokens / max(self.total_texts, 1),
            "avg_texts_per_call": avg_texts_per_call,
            "overall_texts_per_second": overall_texts_per_second,
            "overall_tokens_per_second": overall_tokens_per_second
        }

    def get_call_history(self):
        """Get detailed history of all embedding calls."""
        return self.call_history

    def get_timing_stats(self):
        """Get detailed timing statistics for embedding calls."""
        if not self.call_history:
            return {}
    ...
    def get_performance_stats(self):
        ...
        texts_per_sec = [call["texts_per_second"] for call in self.call_history if call["texts_per_second"] is not None]
        tokens_per_sec = [call["tokens_per_second"] for call in self.call_history if call["tokens_per_second"] is not None]
        chars_per_sec = [call["chars_per_second"] for call in self.call_history if call["chars_per_second"] is not None]
        
        import statistics
        
        stats = {}
        if texts_per_sec:
            stats["texts_per_second"] = {
                "avg": statistics.mean(texts_per_sec),
                "median": statistics.median(texts_per_sec),
                "min": min(texts_per_sec),
                "max": max(texts_per_sec)
            }
        ...
        if chars_per_sec:
            stats["chars_per_second"] = {
                "avg": statistics.mean(chars_per_sec),
                "median": statistics.median(chars_per_sec),
                "min": min(chars_per_sec),
                "max": max(chars_per_sec)
            }
            
        return stats

    def export_call_history(self, filename=None):
        """Export detailed embedding call history to JSON file."""
        import json
        from datetime import datetime
        
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"embedding_call_history_{timestamp}.json"
        ...
        logger.info(f"📋 Embedding call history exported to: {filename}")
        return filename

    def __str__(self):
        usage = self.get_usage()
        if usage['embedding_calls'] == 0:
            return "Embedding Tracker: No calls recorded"
            
        return (
            f"Embedding Calls: {usage['embedding_calls']} | "
            f"Total Time: {usage['total_time']:.2f}s | "
            f"Avg Time: {usage['avg_time_per_call']:.2f}s | "
            f"Total Texts: {usage['total_texts']:,} | "
            f"Total Tokens: {usage['total_tokens']:,} | "
            f"Speed: {usage['overall_texts_per_second']:.1f} texts/s, {usage['overall_tokens_per_second']:.1f} tokens/s | "
            f"Range: {usage['min_time']:.2f}s - {usage['max_time']:.2f}s"
        )
